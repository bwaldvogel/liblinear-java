...
/**
 * <h2>Java port of
 * <a href="http://www.csie.ntu.edu.tw/~cjlin/liblinear/">liblinear</a></h2>
 *
 * <p>
...
public class Linear {
	...
	public static Model loadModel(File modelFile) throws IOException {
		BufferedReader inputReader = new BufferedReader(
				new InputStreamReader(new FileInputStream(modelFile), FILE_CHARSET));
		try {
	...
	private static void solve_l2r_l1l2_svr(Problem prob, double[] w, Parameter param) {
		...
		double Gnorm1_init = -1.0; // Gnorm1_init is initialized at the first
									// iteration
	...
	private static void solve_l1r_l2_svc(Problem prob_col, double[] w, double eps, double Cp, double Cn) {
		...
		for (j = 0; j < w_size; j++) {
			for (Feature x : prob_col.x[j]) {
				x.setValue(x.getValue() * prob_col.y[x.getIndex() - 1]); // restore
																			// x->value
			}
			if (w[j] != 0) {
				v += Math.abs(w[j]);
